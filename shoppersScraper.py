...
async def grabGeolocation(item: Item):
    city = item.city
    state = item.state
    zip = item.zip
    ...
    try:
        ...
        latitude = loc["lat"]
        longitude = loc["lon"]
        return {"Geocode": f"lat: {latitude}, lon: {longitude}"}
...
#     # Set geolocation
#     context.set_geolocation({ "latitude": 39.083997, "longitude": -77.161263})
#     # Grant geolocation
